...
export const AdmissionsTab: React.FC = () => {
  ...
  const filteredAdmissions = admissions.filter((a) => {
    const isPending = a.admissionStatus === 'pending' || (!a.admissionStatus && !a.assignedTeacherId);
    const isVerified = a.admissionStatus === 'verified' || (!isPending && a.admissionStatus !== 'rejected');
    const isRejected = a.admissionStatus === 'rejected';
          ...
          <table className="w-full text-start text-xs">
            <thead className="bg-slate-50 text-slate-600 border-b border-slate-200 font-bold uppercase tracking-wider">
              <tr>
                ...
                <th className="py-3.5 px-4 text-start">Monthly Fee</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredAdmissions.length === 0 ? (
                ...
                filteredAdmissions.map((adm) => {
                  ...
                  const teacher = teachers.find((t) => t.id === adm.assignedTeacherId);
                  const isPending = adm.admissionStatus === 'pending' || (!adm.admissionStatus && !adm.assignedTeacherId);
                  const fee =
                    adm.fee ??
                    (adm.classType === 'group'
                  ...
                  return (
                    <tr key={adm.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="py-3.5 px-4 text-slate-500">
                        {adm.createdAt || adm.admissionDate}
                      </td>

                      <td className="py-3.5 px-4 text-center">
                        <span
                          className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold ${
                            !isPending && adm.admissionStatus === 'verified'
                        ...
                        >
                          {!isPending && adm.admissionStatus === 'verified' && <CheckCircle2 className="w-3 h-3 text-emerald-600" />}
                          {adm.admissionStatus === 'rejected' && <XCircle className="w-3 h-3 text-red-600" />}
                          {isPending && <Clock className="w-3 h-3 text-amber-600" />}
                          <span className="capitalize">{isPending ? 'Pending' : adm.admissionStatus}</span>
                        </span>
                      </td>
          ...
          <div className="relative w-full max-w-xl bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleExecuteVerification} className="p-6 space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {/* Dynamic Fee Display */}
                <div className="col-span-2 p-3 bg-blue-50/70 border border-blue-200 rounded-xl flex items-center justify-between">
                  <div>
                    <span className="text-[11px] font-bold text-blue-900 block">
                      Tuition Fee for {assignClassType === 'group' ? 'Group Class' : 'One-to-One Class'}
                    </span>
                    <span className="text-[10px] text-blue-700">
                      Standard academy fee mapped to this class format
                    </span>
                  </div>
                  <div className="text-right">
                    <span className="text-base font-black text-blue-950">
                      ₹{assignClassType === 'group'
                        ? (courses.find((c) => c.id === assignCourseId)?.groupFee || courses.find((c) => c.id === assignCourseId)?.fee || 500)
                        : (courses.find((c) => c.id === assignCourseId)?.oneToOneFee || ((courses.find((c) => c.id === assignCourseId)?.fee || 600) * 2))}
                    </span>
                    <span className="text-[10px] text-blue-600"> / month</span>
                  </div>
                </div>
